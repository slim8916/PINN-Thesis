...
def PINN_builder(in_shape=2, out_shape=1, layers=default_layers):
    ...
    output_layer = Dense(out_shape, use_bias=False, name='output_layer')(x)
    # building the model
...
@tf.function
def custom_loss(u_pinn, collo_pts_batch, collo_subds_size_batch, diffEq_residu, eps, bds_pts_batch, bds_vals_batch, idx_split_type, type_BC):
    # Collocation loss
    ...
    # Data loss
    #initial condition
    ic_pts_batch=bds_pts_batch[:idx_split_type]
    ic_vals_batch=bds_vals_batch[:idx_split_type]
    ic_loss=tf.reduce_sum(tf.square(u_pinn(ic_pts_batch)-ic_vals_batch))
    bc_pts_batch=bds_pts_batch[idx_split_type:]
    bc_vals_batch=bds_vals_batch[idx_split_type:]
    if  type_BC==0: #Dirichlet
        bc_loss=tf.reduce_sum(tf.square(u_pinn(bc_pts_batch)-bc_vals_batch))
    elif type_BC==1: #Neumann
        t, x = tf.split(bc_pts_batch, num_or_size_splits=2, axis=1)
        with tf.GradientTape() as tape:
            tape.watch([t, x])
            u0 =  u_pinn(tf.concat([t, x], axis=1))
        bc_grad=tape.gradient(u0, x)
        bc_loss=tf.reduce_sum(tf.square(bc_grad-bc_vals_batch))
    #elif type_BC==2: #Periodic
    bds_loss=(ic_loss+bc_loss)/(ic_vals_batch.shape[0]+bc_vals_batch.shape[0])
...
def training_loop(loop_params):
    ...
    tr_idx_split_type = int(bds_size_batch[0].numpy())
    ...
    for epoch in range(num_epochs):
        ...
        with tf.GradientTape(persistent=True) as tape:
            arr_loss=custom_loss(u_pinn, collo_pts_batch, collo_subds_size_batch, heatEq_residu, 0.5, bds_pts_batch, bds_vals_batch, tr_idx_split_type, type_BC)
...
time_t = {
    ...
    'num_subdomains': 7,
    'num_pts_bd': 1000,
    'fct_lower': lambda x: np.sin(2 * x)+(tf.square(x)/2)-1*x
}
types_BC={
    'Dirichlet':0,
    'Neumann':1,
...
sp_x = {
    ...
    'num_subdomains': 1,
    'num_pts_bd': 100,
    'type':types_BC['Neumann'],
    'fct_lower': lambda t: t*0,
    'fct_upper': lambda t: t*0
}
@tf.function
def heatEq_residu(u_pinn, vect):
    ...
    del tape1
    del tape2
    return u_t -alpha*u_xx-tf.exp(-t)*tf.sin(3*x)+tf.cos(np.pi*t)- tf.sin(np.pi*t)-((np.pi*x)/2)*((2-x)*tf.sin(np.pi*t)-x*tf.cos(np.pi*t))
...
for bd in (sp_boundaries):
    if 'fct_lower' in bd:num_pts_bds+=[bd['num_pts_bd']]
    if 'fct_upper' in bd:num_pts_bds+=[bd['num_pts_bd']]
vl_idx_split_type = int(num_pts_bds[1])
type_BC=sp_boundaries[1]['type']
validation_loss = custom_loss(u_pinn, vl_collo_pts, vl_collo_subds_size, heatEq_residu, 0, vl_bds_pts, vl_bds_vals, vl_idx_split_type,type_BC)
...
def opt_LBFGSB(max_iterations=1000):
    ...
    for bd in (sp_boundaries):
        if 'fct_lower' in bd:num_pts_bds+=[bd['num_pts_bd']]
        if 'fct_upper' in bd:num_pts_bds+=[bd['num_pts_bd']]
    idx_split_type = int(num_pts_bds[1])
    type_BC=sp_boundaries[1]['type']
    ...
    def obj_func(weights):
        ...
        with tf.GradientTape() as tape:      
            arr_loss=custom_loss(u_pinn, tr_collo_pts, 
                                 ...
                                 heatEq_residu, loop_params['eps'], tr_bds_pts, tr_bds_vals, 
                                 idx_split_type, type_BC)
